#pragma once
#include "Resource.h"
#include "Framework/singleton.h" // this is new
...
namespace kiko
{
	...
	class ResourceManager : public Singleton<ResourceManager> /// this is new
	{
	public:
	...
	std::shared_ptr<T> ResourceManager::Get(const std::string& filename, TArgs ... args)
	{
		...
		{
			WARNING_LOG("Failed to create resource '%s'." << filename);
			return res_t<T>();
		}
