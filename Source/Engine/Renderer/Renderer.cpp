...
namespace kiko
{
	...
	void Renderer::DrawTexture(Texture* texture, Rect source, const Transform& transform, const vec2& origin,
		bool flipH)
	{
		mat3 mx = transform.GetMatrix();
		...
		// Render the texture with an optional rotation angle
		SDL_RenderCopyEx(m_renderer, texture->GetTexture(), (SDL_Rect*)&source, &dest, 
			RadiansToDegrees(mx.GetRotation()), &center, (flipH) ? SDL_FLIP_HORIZONTAL :SDL_FLIP_NONE);
	}
