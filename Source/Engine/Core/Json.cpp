...
namespace kiko
{
	...
	{
		// check if 'name' member exists and is an array with 4 elements for rgba
	...
	{
		// check if 'name' member exists and is an array with 4 elements for xywh
		if (!value.HasMember(name.c_str()) || !value[name.c_str()].IsArray() || value[name.c_str()].Size() != 4)
		{
			if (required) ERROR_LOG("Cannot read json info: " << name.c_str());
			return false;
	...
	bool Json::Read(const rapidjson::Value& value, const std::string& name, std::vector<std::string>& data,
		bool required)
	{
